  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const arc of arcs) {
        ...
        const pulse = 0.25 + 0.55 * Math.sin(t * arc.sp + arc.ph);
        if (pulse < 0) continue;
        ...
        for (const pt of [pa, pb]) {
          ctx.save();
          ctx.globalAlpha = Math.max(0.12, pulse * 0.4);
          ctx.fillStyle = '#71717a';
          ctx.beginPath();
          ctx.arc(pt.x, pt.y, 1.2 + pulse * 0.8, 0, Math.PI*2);
          ctx.fill();
          ctx.restore();
        }
      }
