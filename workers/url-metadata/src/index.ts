/**
 * URL Metadata API — extract OG, Twitter, and page metadata at the edge.
 */
...
const pageTitle = (html: string) => html.match(/<title[^>]*>([^<]*)<\/title>/i)?.[1]?.trim();
...
async function extract(url: string) {
  const r: Record<string, unknown> & { success: boolean; url: string; fetchedAt: string } = {
    success: false,
    url,
  ...
  try {
    ...
    const res = await fetch(url, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; WorkersCanDo-Metadata/1.0)',
        Accept: 'text/html',
      },
      redirect: 'follow',
    });
    ...
    r.title = meta(html, 'og:title') || (pageTitle(html) ? decode(pageTitle(html)!) : undefined);
    r.description = meta(html, 'og:description') || meta(html, 'description');
    const ogImageRaw = meta(html, 'og:image') || '';
    r.image = resolve(ogImageRaw, url) || undefined;
...
export default {
  async fetch(req: Request): Promise<Response> {
    ...
    if (u.pathname === '/api/metadata') {
      ...
      const status = data.success === false && data.error ? 422 : 200;
      const res = jsonResponse(data, status);
      const headers = new Headers(res.headers);
      headers.set(
        'Cache-Control',
        status >= 400 ? 'private, no-store' : 'public, max-age=3600',
      );
    ...
    return jsonResponse({
      ...
      endpoint: '/api/metadata?url=' + encodeURIComponent('https://example.com'),
    });
  },
};
